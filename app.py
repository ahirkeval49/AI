...
if current_page == "home":
    ...
    <body><script>
        ...
        const agents = [
            {{name: "Auditor", page: "explorer", pos: [10, 5, 2]}},
            {{name: "Dashboard", page: "dashboard", pos: [-10, -5, 4]}},
            {{name: "Strategist", page: "analysis", pos: [2, 9, -6]}},
            {{name: "Knowledge Graph", page: "graph", pos: [6, -8, -4]}}
        ];

        agents.forEach(a => {{
            const el = document.createElement('div'); el.className = 'node-label'; el.innerText = a.name;
            // DESKTOP FIX: Direct top-level navigation to bypass sandbox iframe blocks
            el.onclick = () => {{
                window.top.location.href = window.top.location.origin + window.top.location.pathname + "?page=" + a.page;
            }};
...
# ======================= ARCHITECT DASHBOARD =======================
    ...
    if not f_df.empty:
        ...
        with c_left:
            st.markdown("<div class='console-card'><h3>Campaign Spend Overview</h3>", unsafe_allow_html=True)
            ...
            st.dataframe(spend_table.sort_values('Spend', ascending=False).head(10).style.format({'Spend': '${:,.2f}', 'Budget': '${:,.2f}', 'Budget Utilization': '{:.1%}'}), use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
        with c_right:
...
# ======================= STRATEGIST DEEP-DIVE =======================
    ...
    with c1:
        ...
        fig_p = px.scatter(reg_df, x='Spend', y='Users', hover_name='Display_Name', trendline="lowess", color_discrete_sequence=[CMU_RED])
        fig_p.update_layout(paper_bgcolor=WHITE, plot_bgcolor=WHITE, font_color=TEXT_DARK, xaxis=dict(gridcolor='#eee'), yaxis=dict(gridcolor='#eee'))
        st.plotly_chart(fig_p, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div class='console-card'><h3>True Quality: Lowest CPQM</h3>", unsafe_allow_html=True)
        st.dataframe(master_df.sort_values('CPQM').head(8)[['Display_Name', 'Vendor', 'CPQM', 'Spend', 'Users']].style.format({'CPQM': '${:.2f}', 'Spend': '${:,.0f}'}), use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
# ======================= KNOWLEDGE GRAPH =======================
    ...
    if not master_df.empty:
        ...
        for vend in master_df['Vendor'].unique():
            if str(vend) not in ['0', 'nan', 'Unknown']:
                net.add_node(vend, size=35, color="#555555", label=vend); net.add_edge("CMU", vend)
                for _, r in master_df[master_df['Vendor'] == vend].sort_values('Users', ascending=False).head(8).iterrows():
                    net.add_node(str(r['Display_Name']), size=15, color=CMU_RED, label=str(r['Display_Name'])[:20]); net.add_edge(vend, str(r['Display_Name']))
